...
class ArgConstraint:
    ...
    def validate(self, name: str, value: object) -> None:
        ...
        if self.hasSubClass and not issubclass(type(value), self.subClass):
            raise TypeError(f"{name} must be a subclass of {self.subClass}")

        if self.hasMin and value < self.min:
            raise ValueError(f"{name} must not be less than {self.min}")

        if self.hasMax and value > self.max:
            raise ValueError(f"{name} must not be greater than {self.max}")

        if self.hasMinLength and len(value) < self.minLength:
            raise ValueError(f"{name} must not be shorter than {self.minLength}")

        if self.hasMaxLength and len(value) > self.maxLength:
            raise ValueError(f"{name} must not be longer than {self.maxLength}")
